...
|---|---|---|
| 1 | **Logisim is not in the installer** | measured: with no Logisim the backend falls back to an engine that raises on the first solve. Every question fails and the app looks installed |
| 2 | **The analog loop is not wired into the request path** | `server.py` calls `design.solve` only. Half the lab's experiments are analog, so half the questions get refused by an app that could answer them |
| 3 | **No LTspice detection or first-run guidance** | analog needs the student's own LTspice. Silence here becomes a confusing failure at solve time |
| 4 | **A 30–120 s analog solve has no UI** | a transient run is far slower than a truth table. The page must show progress or it reads as hung |
